...
pred (n : Nat) : Option Nat :=
  if n is s n' then Some n' else None

add (n m : Nat) : Nat :=
  if n is s n' then s (add n' m) else m
```

The `is` syntax of course also works when we want to access constructors' arguments by name.
