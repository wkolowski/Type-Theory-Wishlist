...
For `Bool`, we have two discriminators
...
Discriminators would be very easy to define manually, but the definitions are very boilerplate. For example, `z?` is defined as follows.
...
There is another way to organize the matter of discriminators. Namely, for every inductive type we get an autogenerated type of its constructors. For example, for `Bool` we get the type `BoolConstr` and for `Nat` we get `NatConstr`, which are defined as

```
data BoolConstr : Type
| ff
| tt
...
These types are used in the autogenerated function `whichConstr`, defined as

```
whichConstr-Bool : Bool -> BoolConstr
| ff => ff
| tt => tt

whichConstr-Nat : Nat -> NatConstr
| z   => z
| s _ => s
```

Only then are the discriminators generated.
...
Yet another alternative to deal with discriminators is to introduce an alternative syntax for doing single-case pattern matching. This syntax combines the usual boolean `if _ then _ else` with the new syntactic construct `t is p`, which matches the term `t` against the pattern `p` and returns a boolean.

In this syntax, the discriminators for `Nat` are

```
z? (n : Nat) : Bool := n is z

s? (n : Nat) : Bool := n is s _ // Or alternatively: `n is s`, without the _ at the end.
```
