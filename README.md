...
But we can use the `is` syntax to define much more than just discriminators. In the examples below, we show how to use `is` to define the `pred`ecessor function for naturals and `add`ition as an one-liner.
